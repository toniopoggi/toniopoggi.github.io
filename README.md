# antoniopoggi.com

[antoniopoggi.com](https://antoniopoggi.com) brings together my work on
Omniscope, data analytics, AI, open software and citizen-led air-quality
monitoring.

I am COO of [Visokio](https://visokio.com), the team behind
[Omniscope](https://visokio.com/omniscope). Omniscope is one workspace for the
whole data journey. It lets people connect, prepare, analyse and visualise
data, then deliver trusted outputs without stitching separate tools together.

## Visokio and Omniscope

My work spans the evolution of Omniscope itself and the practical problems it
is built to solve: connecting and preparing data, visual exploration,
interactive reporting, automation, publishing and applied AI.

- [Visokio and Omniscope](https://antoniopoggi.com/visokio/)
- [Writing at Visokio](https://visokio.com/author/antonio17b4b024ad/)

## Air quality and citizen science

In 2019 I used Omniscope to make public air-quality measurements from Taranto
easier for citizens to explore. That work developed into automated benzene
alerts, long-term analysis and collaboration with people campaigning for the
health of Taranto and the Tamburi neighbourhood.

An analysis of more than 80,000 hourly benzene measurements was subsequently
presented by PeaceLink during a hearing of the Italian Senate's Environment
Committee. The [official presentation filed by the
Senate](https://www.senato.it/application/xmanager/projects/leg19/attachments/documento_evento_procedura_commissione/files/000/429/055/2024_02_06_PeaceLink_slides.pdf)
credits the data analysis and processing carried out with Omniscope.

The latest project takes the same principle into open hardware: a customised
AirGradient monitor sends measurements directly and privately to an Omniscope
workflow, without a vendor cloud, middleware service, always-on computer or
inbound router port.

- [The air-quality work](https://antoniopoggi.com/air-quality/)
- [From an open sensor to Omniscope](https://antoniopoggi.com/writing/airgradient-directly-to-omniscope/)
- [AirGradient to Omniscope firmware](https://github.com/toniopoggi/airgradient-omniscope)
- [Android controller](https://github.com/toniopoggi/airgradient-omniscope-android)

## Writing and public work

- [Articles and notes](https://antoniopoggi.com/writing/)
- [LinkedIn](https://www.linkedin.com/in/antoniopoggi)
- [GitHub](https://github.com/toniopoggi)
